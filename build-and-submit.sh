...
if [ -z "$NEW_VERSION" ]; then
  ...
  node -e "
    ...
    appConfig = appConfig.replace(/buildNumber:\s*[\"\'](\d+)[\"\']/, (match, p1) => \`buildNumber: \"\${parseInt(p1) + 1}\"\`);
    appConfig = appConfig.replace(/versionCode:\s*(\d+)/, (match, p1) => \`versionCode: \${parseInt(p1) + 1}\`);
